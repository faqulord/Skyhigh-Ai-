...
mongoose.connect(process.env.MONGO_URL).then(() => console.log("🚀 Rafinált Robot Róka ALMA Build - MOBILE READY"));
...
app.use(session({
    secret: 'skyhigh_alma_mobile_final',
...
async function runAiRobot() {
    try {
        ...
        await new ChatMessage({ sender: 'Róka', text: `Főnök, az elemzés kész! Master Tipp: ${result.match}. 30 napos profit ON.` }).save();
        return true;
...
app.get('/dashboard', async (req, res) => {
    ...
    res.render('dashboard', { user, dailyTip, recommendedStake, displayDate: getHuFullDate(), nextTipText });
});
...
app.post('/admin/chat', checkAdmin, async (req, res) => {
    ...
    const aiRes = await openai.chat.completions.create({
        model: "gpt-4-turbo-preview",
        messages: [{ role: "system", content: "Te vagy a Rafinált Robot Róka." }, { role: "user", content: req.body.message }]
    });
