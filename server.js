...
const FOX_QUOTES = ["FALKA FIGYELEM! Ma nem kérünk... Elveszünk! 🦊💰", "A tőke a lőszer! 💣", "Hideg fej, forró oddsok. 🦊"];
...
mongoose.connect(process.env.MONGO_URL).then(() => console.log(`🚀 RÓKA MOTOR V76 (FIXED) - ONLINE`));
...
app.use(session({
    secret: 'fox_v76_ultimate_fix', resave: true, saveUninitialized: true,
...
// --- DASHBOARD: MINDEN VÁLTOZÓVAL (HOGY NE CRASH-ELJEN) ---
app.get('/dashboard', async (req, res) => {
    try {
        ...
        // MINDEN VÁLTOZÓT ELKÜLDÜNK, HOGY BÁRMELYIK EJS-T HASZNÁLOD, MŰKÖDJÖN
        res.render('dashboard', { 
            user, dailyTip, pendingTips, users, chatHistory,
            suggestedStake: Math.round(bank * p), userBank: bank, 
    ...
    } catch (err) { console.error(err); res.status(500).send("Hiba történt."); }
});

// ADMIN ÚTVONAL (Ugyanazokat az adatokat kapja)
app.get('/admin', checkAdmin, async (req, res) => {
    try {
        ...
        res.render('admin', { 
            user, users, pendingTips, dailyTip, chatHistory, strategyMode: settings.strategyMode, 
            brandName: BRAND_NAME, ownerEmail: OWNER_EMAIL, suggestedStake: Math.round(bank * p), userBank: bank, monthlyProfit: user.monthlyProfit || 0, foxQuotes: FOX_QUOTES
        });
    } catch (err) { res.status(500).send("Hiba az Adminon."); }
});
