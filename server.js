...
mongoose.connect(process.env.MONGO_URL).then(() => console.log("🚀 Skyhigh Neural Engine v4.0 Online"));
...
async function runAiRobot() {
    try {
        ...
        const matches = fixtures.map(m => `${m.teams.home.name} vs ${m.teams.away.name}`).join(", ");
