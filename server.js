...
const foxQuotes = [
    ...
    "👑 Ne elégedj meg az apróval. Te a Falka tagja vagy. Nekid a trón jár!"
];

// --- ADATBÁZIS MODELL DEFINÍCIÓK ---
...
// --- APP KONFIGURÁCIÓ ---
mongoose.connect(process.env.MONGO_URL).then(() => console.log(`🚀 ${BRAND_NAME} System Ready - v29.1 PRO`));
...
app.use(session({
    secret: 'skyhigh_boss_system_secret_v29_1', resave: true, saveUninitialized: true,
...
// --- ÚJ, TŰPONTOS AI MOTOR (13:00 - 23:59) ---
async function runAiRobot() {
    ...
    await logToChat('Róka', `📊 Mód: ${strategyMode} | Cél: Havi Profit Növelése`);
    ...
    try {
        ...
        if (fixtures.length > 0) {
            validFixtures = fixtures.filter(f => {
                ...
                return hunHour >= 13 && hunHour <= 23; // CSAK 13:00 UTÁNI MECCSEK
            });
        }
        ...
        if (validFixtures.length > 0) {
            ...
            // HA NINCS MAI MECCS, MEGNÉZZÜK A HOLNAPOT
            const tomorrow = new Date(); tomorrow.setDate(tomorrow.getDate() + 1);
            const tomorrowStr = tomorrow.toLocaleDateString('en-CA');
            await logToChat('Róka', `🔎 Ma nincs alkalmas meccs, nézem a holnapi (${tomorrowStr}) kínálatot...`);
        ...
        const analysisPrompt = `SZEREP: Profi Matematikai Sportelemző Robot. FELADAT: Válaszd ki az EGYETLEN LEGBIZTOSABB meccset a listából. KERÜLD A SZERENCSÉT. MÓD: ${strategyMode}. FORMAT (JSON): { "league": "...", "match": "Hazai - Vendég", "prediction": "Tipp", "odds": "1.XX", "reasoning": "Logikai indoklás", "profitPercent": 5, "matchTime": "ÓÓ:PP", "matchDate": "${targetDate}", "bookmaker": "Bet365", "stake": "${stakeAdvice}" }`;
        ...
        const marketingPrompt = `Meccs: ${result.match}, Tipp: ${result.prediction}. Írd át Zsivány Róka stílusban tagoknak. Kezdd így: 📅 MA ${result.matchTime} - ...`;
...
app.post('/admin/chat', checkAdmin, async (req, res) => {
    try {
        ...
        const systemPrompt = `Te vagy a Zsivány Róka AI. Mai tipp: ${todayTip ? todayTip.match : "Nincs még"}. Havi profit: ${stat ? stat.totalProfit : 0}%. Beszélj emberként a Főnökkel, légy szakmai és magabiztos. A cél a profit!`;
...
app.post('/admin/social-content', checkAdmin, async (req, res) => {
    ...
    const prompt = req.body.type === 'win' ? `Írj IG sztorit a nyereményről: ${tip?.match}. Zsivány stílus!` : `Motiváció sportfogadóknak.`;
...
app.post('/admin/refine-text', checkAdmin, async (req, res) => {
    ...
    const aiRes = await openai.chat.completions.create({ model: "gpt-4-turbo-preview", messages: [{ role: "system", content: "Marketing Copywriter." }, { role: "user", content: `Írd át röviden Zsivány stílusban, kezdd az időponttal (${tip.matchTime}): ${tip.memberMessage}` }] });
    tip.memberMessage = aiRes.choices[0].message.content; await tip.save();
    res.redirect('/admin');
});
...
app.post('/admin/activate-user', checkAdmin, async (req, res) => {
    ...
    await User.findByIdAndUpdate(req.body.userId, { hasLicense: true, licenseExpiresAt: exp });
    res.redirect('/admin');
});

// AUTH & BASIC ROUTES
app.post('/auth/register', async (req, res) => { 
    if(!req.body.terms) return res.send("Hiba"); 
    const h=await bcrypt.hash(req.body.password,10); 
    try { const u=await new User({fullname:req.body.fullname,email:req.body.email.toLowerCase(),password:h}).save(); req.session.userId=u._id; res.redirect('/pricing'); } 
