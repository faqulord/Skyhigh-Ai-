...
mongoose.connect(process.env.MONGO_URL).then(() => console.log(`🚀 RÓKA MOTOR V38 (SAFE MODE) - ONLINE`));
...
app.use(session({
    secret: 'fox_v38_safe', resave: true, saveUninitialized: true,
...
app.post('/admin/social-content', checkAdmin, async (req, res) => {
    // ITT A JAVÍTÁS: "Magabiztos" nem "agresszív"
    const prompt = req.body.type === 'win' 
        ? "Te vagy a Zsivány Róka. Írj egy nagyon magabiztos, dörzsölt Instagram posztot a mai nyereményről! Használj emojikat. Stílus: profi, okos." 
        : "Te vagy a Zsivány Róka. Írj egy motivációs posztot arról, hogy a fegyelem pénzt terem.";
    try {
    ...
    } catch(e) { res.json({ content: "Az AI pihen." }); }
});
