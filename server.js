...
mongoose.connect(process.env.MONGO_URL)
    .then(() => console.log(`🚀 ${BRAND_NAME} System Ready - HUNGARIAN TIME FIX`))
...
app.use(session({
    secret: 'skyhigh_boss_system_secret_v15_hun',
...
// --- AI MOTOR (BUDAPEST IDŐZÓNA + 3 ÓRA PUFFER) ---
    ...
    try {
        ...
        if (response.data.errors && Object.keys(response.data.errors).length > 0) {
            ...
            if (errStr.includes("suspended")) {
                await new ChatMessage({ sender: 'System', text: `⛔ KRITIKUS HIBA: A rendszer még mindig a RÉGI, LETILTOTT kulcsot látja! Kérlek, a Railway-en nyomj egy "REDEPLOY"-t (Újratelepítést), hogy frissüljön!` }).save();
                return false;
            }
        }
        ...
        // --- 2. IDŐSZŰRÉS (BUDAPEST IDŐ + 3 ÓRA) ---
        const now = new Date();
        ...
        if (fixtures.length > 0) {
            // Szűrés: Csak olyan meccs, ami KÉSŐBB kezdődik, mint 3 óra múlva
        ...
        // 4. ADATOK FORMÁZÁSA (BUDAPEST IDŐZÓNA!)
        const matchData = validFixtures.slice(0, 30).map(f => {
            // Itt történik a varázslat: UTC -> Budapest
